...
public class ReserveInMapper {
	...
	public static ReserveDTO from(Reserve reserve) {
		...
		return ReserveDTO.builder().reserveID(reserve.getKey().toString()).dateIn(reserve.getDateIn())
				.dateOut(reserve.getDateOut()).state(reserve.getState()).build();
	}

	public static Reserve from(ReserveDTO reserveDTO) throws BussinessRuleValidationException {
