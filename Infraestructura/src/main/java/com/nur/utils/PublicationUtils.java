...
public class PublicationUtils {
    ...
    public static List<Publication> publicationJpaModelList(
            List<UserPublicReserveJpaModel> publications
    ) throws BussinessRuleValidationException  {
        if (publications == null) return Collections.emptyList();
        return publications
                .stream()
                .map((UserPublicReserveJpaModel publication) -> {
                    try {
    ...
    public static Publication jpaToPublication(UserPublicReserveJpaModel jpaModel)
        ...
        return new Publication(jpaModel.getId(),jpaModel.getAmount(), jpaModel.getPublishID(), jpaModel.getReserveID(), jpaModel.getUserID());
    }
}
