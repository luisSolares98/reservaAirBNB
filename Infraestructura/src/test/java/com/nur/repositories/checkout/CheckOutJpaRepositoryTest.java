...
class CheckOutJpaRepositoryTest {
	...
	void update() throws BussinessRuleValidationException, ParseException {
		...
		Mockito.when(reserveCrudRepository.findById(any(UUID.class)))
				.thenReturn(Optional.of(ReserveFixture.whitDefaultJPA()));
	...
	void getById() throws BussinessRuleValidationException, ParseException {
		...
		UUID sampleId = UUID.randomUUID();
		Mockito.when(checkOutCrudRepository.findById(sampleId))
				.thenReturn(Optional.of(CheckOutFixture.whitDefaultJPA()));
